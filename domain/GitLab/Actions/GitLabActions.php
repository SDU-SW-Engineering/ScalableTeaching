...
class GitLabActions implements SourceControl
{
    public function showProject(string $id, string $user = 'default'): ?Project
    {
        ...
        $nodeSelector->selectRepository()
            ...
            ->selectLastCommit()
            ->selectSha();
        $client = new Client(config('scalable.gitlab_url') . '/api/graphql', ["Authorization" => 'Bearer ' . $token]);
    ...
    public function currentUser(string $user = 'default'): User
    {
        ...
        $client = new Client(config('scalable.gitlab_url') . '/api/graphql', ["Authorization" => 'Bearer ' . User::token($user)]);
