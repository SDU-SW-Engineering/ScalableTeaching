...
class GitLabActions implements SourceControl
{
    ...
    public function currentUser(string $user = 'default'): User
    {
        ...
        $client = new Client(config('GITLAB_URL') . '/api/graphql', ["Authorization" => 'Bearer ' . User::token($user)]);
    ...
    public function getFilesFromDirectories(string|int $projectId, DirectoryCollection $directoryCollection, ?string $ref = null): void
    {
        ...
        $client = new Client(config('GITLAB_URL') . '/api/graphql', ["Authorization" => 'Bearer ' . config('scalable.gitlab_token')]);
