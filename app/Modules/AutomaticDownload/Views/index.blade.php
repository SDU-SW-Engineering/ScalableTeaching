                    ...
                    @if($task->has_ended)
                        @if($download['download'])
                            @if($download['download']->state == \App\Models\Enums\DownloadState::OnDisk)
                                ...
                                <td class="flex px-6 py-4 gap-2 justify-end">
                                    <a target="_blank" href="{{ route('courses.tasks.show-editor', [$course, $task, $download['project'], $download['download']]) }}"
                                       class="p-1.5 hover:bg-lime-green-500 text-white rounded-sm bg-lime-green-400">Open</a>
