...
class OverviewController extends Controller
{
    ...
    {
        if ($task->type->value == 'assignment')
        {
            return $this->assignmentIndex($course, $task);
        }
        return $this->exerciseIndex($course, $task);
    }

    public function assignmentIndex(Course $course, Task $task) : View
    {
    ...
    public function exerciseIndex(Course $course, Task $task) : View
    {
        $projectCount = $task->projects()->count();
        $projectsToday = $task->projects()->whereRaw('date(created_at) = ?', now()->toDateString())->count();
        $finishedCount = $task->projects()->where('status', 'finished')->count();
        ...
        $totalVisitsPerDay = $task->dailyVisits(true);
        $totalVisitsPerDayGraph = $totalVisitsPerDay == null ? null : new Graph(
            $totalVisitsPerDay->keys(),
            new BarDataSet("Visits", $totalVisitsPerDay->values(), "#266ab0")
        );
