            ...
            <div class="flex">
                @if(request()->has('type'))
                    <form class="flex flex-col gap-4 w-full" method="post"
                          action="{{ route('courses.tasks.admin.addDelegation', [$course, $task, 'pool' => request('type')]) }}">
                        @csrf
                        <div class="flex gap-4">
                            <div class="w-1/2">
                                ...
                                @else
                                    <span class="text-sm font-bold dark:text-white">Users</span>
                ...
                @else
                    <div class="flex w-full gap-2">
                        <a href="{{ route('courses.tasks.admin.gradingDelegate', [$course, $task, 'type' => 'role']) }}" class="w-1/2 bg-lime-green-600 py-2 text-gray-100 text-center rounded-md hover:bg-lime-green-700 transition-colors">Role based</a>
                        <a href="{{ route('courses.tasks.admin.gradingDelegate', [$course, $task, 'type' => 'user']) }}" class="w-1/2 bg-lime-green-600 py-2 text-gray-100 text-center rounded-md hover:bg-lime-green-700 transition-colors">User based</a>
                    </div>
                @endif
            </div>
        </div>
