            ...
            <div v-else-if="step === 2" class="flex flex-col items-center h-full justify-center gap-8">
                <div class="flex gap-8">
                    <div v-if="type === 'exercise'" @click="backedBy = 'text'"
                        ...
                        <h2 :class="[ this.backedBy === 'text' ? 'text-white' : 'text-lime-green-500 dark:text-lime-green-400']"
                            class="text-2xl">Text backed</h2>
                        <p :class="[ this.backedBy === 'text' ? 'text-gray-100' : 'text-gray-700 font-thin dark:text-gray-300']"
                           class="font-thin">Simple tasks that can be described via text.</p>
                    </div>
                ...
                <button v-if="canContinueToStep3" @click="goToStep3()"
                        class="bg-lime-green-300 text-white flex items-center justify-center py-2 px-4 rounded hover:bg-lime-green-400">
                    Continue
                </button>
            </div>
            <div v-else-if="step === 3" class="flex flex-col items-center h-full justify-center">
                <h3 class="text-black dark:text-white mb-4 text-1xl">What's the name of the task?</h3>
                ...
                <button @click="save()"
                    ...
                    <span v-if="!loading" class="text-white">Create</span>
                </button>
            </div>
        </div>
    </div>
