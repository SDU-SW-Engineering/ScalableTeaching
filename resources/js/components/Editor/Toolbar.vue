    ...
    <div class="flex h-12 bg-lime-green-700">
        ...
        <div class="w-full justify-between flex">
            ...
            <div class="flex" v-if="context !== 'view'">
                ...
                <a v-bind:href="downloadLink">
                    <button v-if="true" class="transition-colors h-full hover:bg-lime-green-800 text-white px-4 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" fill="currentColor" stroke="currentColor" class="w-5 h-5 mr-2">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M21 21H3M18 11L12 17M12 17L6 11M12 17V3"/>
                        </svg>
                        <span>Download</span>
                    </button>
                </a>
                <button v-if="maxPoints != null && currentPoints != null && delegation.grading" @click="$emit('toggleSubTasks')" class="transition-colors h-full hover:bg-lime-green-800 text-white px-4 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 mr-2">
                ...
                <button v-if="delegation.feedback" @click="$emit('toggleFeedback')" class="transition-colors h-full hover:bg-lime-green-800 text-white px-4 flex items-center">
                ...
                <button @click="submit" class="transition-colors h-full hover:bg-lime-green-800 text-white px-4 flex items-center" v-if="context !== 'recipient'">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 mr-2">
