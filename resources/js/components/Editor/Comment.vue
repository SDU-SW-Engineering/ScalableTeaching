    ...
    <div class="mb-1 rounded-md flex flex-col" :style="{ marginLeft: indentation + 'ch', width: '360px'}">
        <div class="flex justify-between items-center bg-black rounded-t-md p-2 text-white border border-gray-600">
            <div class="flex items-center">
                ...
                <span class="text-xs text-white bg-red-500 rounded-sm p-0.5 ml-1.5 mr-1"
                      v-if="comment.status === 'rejected'">Rejected</span>
                <span class="text-xs text-white bg-yellow-600 rounded-sm p-0.5 ml-1.5 mr-1"
                      v-else-if="comment.status === 'pending'">Pending review</span>
                <span class="text-xs text-white bg-lime-green-600 rounded-sm p-0.5 ml-1.5 mr-1"
                      v-else-if="comment.status === 'approved'">Approved</span>
            </div>
        ...
        <div v-if="actions && isModerated"
             class="flex bg-black rounded-b-md border-b border-r border-l border-gray-600">
            <button :class="[markedAs == 'helpful' ? 'text-yellow-300 ' : 'text-gray-300']" @click="markAs('helpful')"
                    v-if="perspective === 'recipient'"
            ...
            <button :class="[markedAs == 'helpful' ? 'text-gray-300' : 'text-yellow-300']"
                    @click="markAs('not_helpful')" v-if="perspective === 'recipient'"
                    class="w-full flex flex-col items-center py-2 last:rounded-br-md hover:bg-gray-900">
        ...
        <div class="bg-black rounded-b border-r border-l border-b border-gray-600 flex flex-col p-3"
             v-if="comment.reviewer_feedback != null">
